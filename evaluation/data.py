    ...
    **Custom Data**
    ...
    def load_docs(
        ...
        if self.dataset == "trump":
            ...
            self.docs, self.timestamps = self._total()
    ...
    def _total(self) -> Tuple[List[str], List[str]]:
        """Prepare the total dataset"""
        ...
        dataset = pd.read_csv(
            "https://drive.google.com/file/d/19prq9ohZYMRJ6CZuO5kTyiWeYDVuDfO0/view?usp=share_link"
        )
        dataset["comment"] = dataset.apply(lambda row: create_paragraphs(row.comment), 1)
        dataset = dataset.explode("comment").sort_values("date")
        docs = dataset.comment.tolist()
